...
export function makeDaiMarketOverview({
  ...
  const skyCapacity = NormalizedNumber(D3MInfo.maxDebtCeiling.minus(D3MInfo.D3MCurrentDebtUSD))
  const marketSize = NormalizedNumber(reserve.totalLiquidity.plus(skyCapacity))
  const totalAvailable = NormalizedNumber(marketSize.minus(reserve.totalDebt))
